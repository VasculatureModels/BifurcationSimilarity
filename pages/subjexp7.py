...
dash.register_page(__name__, path='/pages/subjexp7')



lines = []
# Read image playlist :
with open(r"playlist7.txt", 'r') as fp:
	lines = fp.readlines()
...
def update_vtk_view(val1, val2, val3, val4, val5, val6, val7, val8, val9, val10):
	if (val1 == 0) and (val2 == 0) and (val3 == 0) and (val4 == 0) and (val5 == 0) and (val6 == 0) and (val7 == 0) and (val8 == 0) and (val9 == 0) and (val10 == 0):
		...
		if "end.nrrd" in volname or idx == len(lines):
			...
			NewScores.sort()
			with open(r'./outputs/output_7_' + str(today) + '_' + current_time + '.txt', 'w') as fp:
